  ...
  namespace :image_services do
    ...
    task :dump_metadata, [:image_service, :base_directory] => :environment do |t, args|
      ...
      lentilService.images.unscoped.find_each do |image|
        ...
        jsonobj["tags"] = image.tags.all.serializable_hash
        jsonobj["licenses"] = image.licenses.all.serializable_hash
        jsonobj["licenses"].each do |lic|
          lic.delete("id")
        end

        jsonobj["like_votes"] = image.like_votes.all.serializable_hash
        jsonobj["flags"] = image.flags.all.serializable_hash
