if defined?(ActiveAdmin)
  ActiveAdmin.register Lentil::Image do
    ...
    collection_action :moderate_flagged do
      ...
      @images = Lentil::Image.joins(:flags).includes(:user, :taggings, :tags).group("lentil_images.id").having("count(lentil_flags.id) > 0").where(:second_moderation => false).paginate(:page => params[:page], :per_page => 10)
      render "/admin/lentil_images/moderate"
    end
    ...
    collection_action :flagging_history do
      @images = Lentil::Image.joins(:flags).includes(:user, :taggings, :tags, :flags).group("lentil_images.id").having("count(lentil_flags.id) > 0").paginate(:page => params[:page], :per_page => 10)
    end
